...
6. **내보내기·백업** (M3)
   - 상세 화면 **Markdown 내보내기**: 분석 결과 + 전사문을 `.md` 파일로 저장 (`YYYYMMDD_HHMM_모드_제목.md`)
   - 설정 탭 **데이터 관리**: 전체 기록 **JSON 백업**(`llmn_backup_YYYYMMDD.json`) / **복원**(id 기준 병합) / **모든 기록·녹음 삭제**
   - 백업/복원은 **기록만** 다룬다 — 녹음 오디오·설정·API 키는 백업에 포함되지 않는다(키 유출 방지).
...
- **M3 (완료):** Markdown 내보내기, JSON 백업/복원(기록만, id 병합), 데이터 전체 삭제, 오프라인 셸 캐시(네트워크 우선 + 3초 타임아웃 폴백, v4)
